...
public class LocalDynamoDBCreationRule extends ExternalResource {
    ...
    private int localDbPort = 19444;

    // private DynamoDBProxyServer server;
    //  private AmazonDynamoDB amazonDynamoDB;

    private DynamoDBProxyServer localDb;
    ...
    protected void before() throws Throwable {
        try {
            ...
            localDb = new DynamoDBProxyServer(port, new LocalDynamoDBServerHandler(new LocalDynamoDBRequestHandler(0, true, null, true, true), null));
            localDb.start();

            dbClient = new AmazonDynamoDBClient(new BasicAWSCredentials("access", "secret"));
            dbClient.setSignerRegionOverride("us-east-1");
            dbClient.setEndpoint("http://localhost:" + port);
        } catch (Exception e) {
